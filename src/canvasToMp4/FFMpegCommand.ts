class FFmpegCommand {
  ...
  constructor(config:any){
    ...
    let title = '';
    if(config.title) title = '-metadata title="'+config.title+'"';
    ...
    if(encoderType == 'libx264'){
      ...
      this.values = ('-hwaccel cuvid -y -f rawvideo -r '+fps+' -s '+width+'x'+height+' -pix_fmt rgb0 -i - -c:v h264_nvenc '+vflip+' '+title+' -pixel_format yuv420p -rc constqp -preset slow -profile:v baseline -crf 18 '+fileName).split('  ').join(' ').split(' '); //ok
